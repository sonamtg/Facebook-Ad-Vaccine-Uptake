...
# For reproducibility, the random number is generated using random.org
seed <- 32055016
set.seed(32055016)
...
baseline_surv <- tibble(
  ...
  # Date when the baseline survey is completed, the range is distributed by day
  baseline_date = sample(seq(ymd("2021-05-01"), ymd("2021-05-15"), by = "day"),
                         size = total_particip, replace = TRUE),
  # The age distribution is right-skewed with few older people in the survey, and the average age is 40
  # Min Age = 18 and Max Age = 90
  age = round(rgamma(total_particip, shape = 6, rate = 0.15)) %>%
    pmax(18) %>%
    pmin(90)) %>%
  mutate(
    # Gender distribution based on the age range
    # Trying to match the real-world scenario where more people who identify themselves as non-binary
    gender = case_when(
      age <= 25 ~ sample(gender,
                         prob = c(0.44, 0.46, 0.10),
                         size = n(), replace = TRUE),
      age <= 40 ~ sample(gender,
    ...
    # Race distribution trying to match the US race distribution
    ...
    education = case_when(
      age <= 25 ~ sample(educ_levels, prob = c(0.1, 0.3, 0.25, 0.31, 0.04),
                         size = n(), replace = TRUE),
      age <= 40 ~ sample(educ_levels, prob = c(0.15, 0.26, 0.24, 0.27, 0.08),
                         size = n(), replace = TRUE),
      TRUE ~ sample(educ_levels, prob = c(0.18, 0.28, 0.20, 0.29, 0.05),
                    size = n(), replace = TRUE)
    ),
    # People atmost 25, the probability of being married = 0.15
    # People who are older than 25 and le
    married = case_when(
      age <= 25 ~ rbinom(n(), 1, 0.15),
      age <= 40 ~ rbinom(n(), 1, 0.74),
      TRUE ~ rbinom(n(), 1, 0.82)
    ),

    # COVID-19 Concern distribution
    ...
    trust_eff = case_when(
      vaccine_trust == "Trust" ~ 0.05,
      vaccine_trust == "No trust" ~ -0.03,  # Penalty
      TRUE ~ 0
    ),

    # Combine (max 25% individual probability)
    prob_vax = pmin(base_prob + educ_eff + trust_eff, 0.15),
    vaccinated = rbinom(n(), 1, prob_vax),
    ...
    fb_active = case_when(
      age <= 25 ~ sample(fb_freq, prob = c(0.18, 0.33, 0.25, 0.24),
                         size = n(), replace = TRUE),
      age <= 40 ~ sample(fb_freq, prob = c(0.37, 0.34, 0.18, 0.11),
                         size = n(), replace = TRUE),
